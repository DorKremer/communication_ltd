...
router.post('/addClient', function (req, res) {
    ...
    const selectQuery = 'SELECT id FROM clients WHERE email = \'' + email + '\'';
    ...
    db.clientDbConfig.query(selectQuery, function (error, results, fields) {
        if (error) {
            ...
            db.clientDbConfig.query('INSERT INTO clients (firstname, lastname, email, phoneNumber) VALUES (?, ?, ?, ?)',
                ...
                function (error, results, fields) {
                    if (error) {
                        ...
                        res.status(200).send(('Client added successfully', firstname));
                    }
                }
            );
        }
